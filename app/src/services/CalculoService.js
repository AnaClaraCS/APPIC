...
/*
  As coordenadas são calculadas usando como peso a quantidade de leituras iguais a um local
...
class CalculoService {
  ...
  const locaisMaisFrequentes = locaisComContagem.sort((a, b) => b.contagem - a.contagem).slice(0, 3);
    ...
    for (const localContagem of locaisMaisFrequentes) {
      ...
      somaX += local.x * localContagem.contagem;
      somaY += local.y * localContagem.contagem;
      somaAndar += local.andar * localContagem.contagem;
      total += localContagem.contagem;
    }
